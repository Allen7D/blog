...
<body>
<!--[if lte IE 8]>
<p class="browserupgrade">您的浏览器版本太旧, 可能会对您的计算机安全造成威胁，请到 <a href="http://browsehappy.com/">这里</a> 更新, 以获取最佳的体验</p>
<![endif]-->
<header>
    <nav class="top">
    ...
    <nav class="main">
        <a href="#" class="brand"><img src="image/logo@1x.png" alt="回到首页"></a>
        <ul>
            <li><a href="#">首页</a></li>
            <li><a href="#">理财频道</a></li>
            <li><a href="#">投资频道</a></li>
...
<div class="container">
    ...
    <section class="ad">
        <div class="item">
            <picture>
                <img src="image/ad001.png" alt="2015年度报告">
            </picture>
        </div>
        <div class="item">
            <picture>
                <img srcset="image/ad002.png" alt="新年红包">
            </picture>
        </div>
        <div class="item">
            <picture>
                <img srcset="image/ad003.png" alt="新手秘籍">
            </picture>
        </div>
    </section>
...
body {
    /*字体大小：12px; 行间距：1.5倍行间距*/
    /*rem继承于根标签「html」*/
    font-size: 1.2rem;
    line-height: 1.5;
    background-color: #f7f7f7;/*浅灰色的背景色，接近白色*/
}
...
header .top {
    line-height: 3rem; /* 撑起字体的高度 */
    padding: 0 1.5rem; /* 内部标签 左右 离网页的间距：15px */
    color: #fff;
    background: #555;
    border-bottom: 1px #ecc000 solid; /* 下边界 */
}
...
header .top ul li {
    display: inline-block; /* 列表布局：放在同一行，且会自动换行 */
}
...
header .top ul li a {
    display: inline-block; /* 或block; 块级元素，才能设置宽高 */
    padding: 0 1.5rem; /* 在li中左右撑开 */
    color: #fff;
 }
...
.transaction {
    /*
        width是「box的content」的宽度；
        不写，则默认为auto继承父标签；
        auto，使得padding可以挤压content
     */
    width: 100%; /*padding: 0 10%; 用padding可以取代「多层包裹」所产生的间隔效果*/
...
.trans-content {
    /* 左右间隔的方式之一 */
    max-width: 80%;
    margin: 0 auto; /* 左右则根据宽度自适应相同值（即居中） */
    font-size: 1.5rem;
}
...
.trans-data {
    float: left;
    padding: 1rem;
    text-align: left; /* 块级元素内文本的水平对齐方式 */
}

.trans-data span {
    /* section的高度：trans-data 的padding的高度 +  span的line-height高度*/
    display: inline-block;
    line-height: 3rem;
    color: #fff;
}
...
.trans-data span.trans-money {
    ...
    padding: 0 1rem;
    border-radius: 3px;
    background-color: rgba(255, 255, 255, 0.9); /* 背景色透明色；opacity会改变所有的透明 */
    box-shadow: 1px 2px 1px #333 inset; /* 添加阴影；内阴影:挖空内嵌的效果（inset） */
}
...
.feature .item {
    display: inline-block;
    width: calc(33.33333% - 3rem); /* CSS3的动态计算方法，3等分；减去左右1rem，再减去间隔的1rem */
    padding: 1rem 1rem 1rem 1rem;  /* 左右都是1rem */
    font-size: 1.4rem;
...
.feature .item + .item {
    margin-left: 1.5rem; /* 2个间隔得到3rem，平均为1rem */
}
...
.notice a:first-child {
    text-overflow: ellipsis;  /* 省略号隐藏文字 */
...
.detail-content .brand {
    float: left;
    display: block;
    -webkit-filter: grayscale(100%); /* 前缀写法(用于支持不同的浏览器) */
    filter: grayscale(100%); /* 灰色的滤镜 */
    width: 20%;
    height: 35px;
...
footer ul li .icon {
    display: inline-block;
    text-indent: -999rem;/* 通过文字缩进，隐藏文字 */
    margin-right: 1.5rem;
    background: url(https://i.alipayobjects.com/e/201307/jebmKcbrh.png) no-repeat; /* 雪碧图 */
...
footer ul li .icon-zfywxk {
    width: 38px;
    height: 27px;
    background-position: -369px -45px;/* 定位 */
}
